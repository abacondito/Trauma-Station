    ...
    public override void Update(float frameTime)
    {
        ...
        while (rotQuery.MoveNext(out var uid, out var rotting, out var perishable, out var xform))
        {
            ...
            if (TryComp<RotIntoComponent>(uid, out var rotInto))
            {
                ...
                {
                    // <Trauma> - swap rotted spawn into the container properly
                    var coords = xform.Coordinates;
                    var spawned = Spawn(rotInto.Entity, coords);
                    if (_container.TryGetContainingContainer((uid, xform, null), out var container))
                    {
                        _container.Remove((uid, xform, null), container, force: true);
                        _container.Insert(spawned, container, force: true);
                    }
                    // </Trauma>
                    QueueDel(uid);
                    continue;
