...
public abstract partial class SharedGunSystem
{
    ...
    private void ManualCycle(Entity<BallisticAmmoProviderComponent> ent, MapCoordinates coordinates, EntityUid? user = null, GunComponent? gunComp = null)
    {
        ...
        Cycle(ent, coordinates, user); // Trauma - added user
    ...
    /// </summary>
    protected void Cycle(Entity<BallisticAmmoProviderComponent> ent, MapCoordinates coordinates, EntityUid? user = null)
    {
    ...
    private void OnBallisticTakeAmmo(Entity<BallisticAmmoProviderComponent> ent, ref TakeAmmoEvent args)
    {
        for (var i = 0; i < args.Shots; i++)
        {
            ...
            if (ent.Comp.Entities.Count > 0)
            {
                ...
                // <Trauma> - check AutoCycle before removing spent ammo
            ...
            else if (ent.Comp.UnspawnedCount > 0)
            {
                ...
                DirtyField(ent.AsNullable(), nameof(BallisticAmmoProviderComponent.UnspawnedCount));
                // <Trauma>
                ammoEntity = PredictedSpawnAtPosition(ent.Comp.Proto, args.Coordinates); // predicted this shit
                // put spent ammo back in the gun if it doesn't autocycle
                if (!ent.Comp.AutoCycle)
                {
